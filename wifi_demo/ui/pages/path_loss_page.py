...
class PathLossPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self._refresh_btn.setToolTip(
            ...
            "(use this after applying a correction to see the updated values)"
        )
    ...
    def _load_file(self, path: str, _silent: bool = False) -> None:
        ...
        ----------
        _silent : bool
            When True, skips the validation popups and does not emit
            file_loaded. Used internally by reload_from_disk() so that
            refreshing an already-loaded file doesn't spam the user with
            the same warning dialog every time, and doesn't trigger a
            reload loop with CorrectionsPage.
        """
        # Guard against a cross-page signal loop: if this exact file is
        # already the one loaded here, skip re-validating/re-emitting.
        # (PathLossPage <-> CorrectionsPage notify each other on load.)
        same_file = (
            self._filepath
            and Path(path).resolve() == Path(self._filepath).resolve()
        )
        if same_file and not _silent:
            return

        # ── Pre-flight validation ────────────────────────────────────────────
        try:
            pl_warnings = PathLossValidator.validate_file(path)
        except PathLossValidationError as ve:
            if not _silent:
                QMessageBox.warning(self, "Path-Loss File Cannot Be Loaded", str(ve))
            return

        if pl_warnings and not _silent:
            warn_txt = "\n".join(f"• {w.message}" for w in pl_warnings)
            QMessageBox.information(
                self, "Path-Loss File Loaded With Warnings",
                f"The file was loaded, but the following issue(s) were found:\n\n"
                f"{warn_txt}"
            )
    ...
    def _refresh(self) -> None:
        """
        Reload the currently open file from disk (toolbar button).
        Lets the operator see path-loss values before/after a correction
        has been applied from the Corrections page, without re-browsing.
        """
        ...
        if self._unsaved:
            reply = QMessageBox.question(
                ...
                "You have unsaved edits in this table. Refreshing will "
        ...
        self._status_lbl.setText(
            self._status_lbl.text() + "   (refreshed)"
        )

    def reload_from_disk(self) -> None:
        """
        Public API — silently reload the currently open file from disk,
        without re-running pre-flight validation popups and without
        re-emitting file_loaded (avoids a reload loop when called in
        response to CorrectionsPage applying a correction).
        Does nothing if no file is currently loaded.
        """
        if not self._filepath:
            return
