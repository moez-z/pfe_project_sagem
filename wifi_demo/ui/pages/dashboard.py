...
class DashboardPage(QWidget):
    ...
    def refresh(self):
        """Re-render the KPI cards and banner from the last loaded report."""
        if self._last_report is not None:
            self._render_report(self._last_report)
        else:
            self._console.setPlainText(
                "No calibration result yet — run a calibration first."
            )
...
class _ResultBanner(QWidget):
    ...
    def set_result(self, report):
        """
        report: CalibrationReport, or None for the initial/idle state.

        Three distinct outcomes are shown, not two:
          - PASS              → everything within limits and tolerance
          - NEEDS CORRECTION  → delta(s) out of tolerance, but no hard
                                 power-limit violation (correctable via EEPROM)
          - FAIL              → at least one block measured outside the
                                 station's hard power limits, or an RX
                                 PER failure — a real hardware failure
        """
        if report is None:
            self._set_style("idle")
            return

        has_hard_fail = report.tx_fail_count > 0 or report.rx_fail_count > 0
        needs_correction = report.tx_needs_correction_count > 0

        if has_hard_fail:
            state = "fail"
            self._text.setText("OVERALL RESULT:  ❌  FAIL — outside hard limits")
        elif needs_correction:
            state = "correction"
            self._text.setText("OVERALL RESULT:  ⚠  NEEDS CORRECTION")
        else:
            state = "pass"
