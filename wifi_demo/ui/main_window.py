...
<<<<<<< HEAD
            ...
            try:
                LogValidator.validate(origin, dut)
            except LogValidationError as ve:
...
<<<<<<< HEAD
        ...
        self._corr_page.path_loss_loaded.connect(
            self._path_loss_page.load_file
        )
        # When a correction is applied (writes updated values to the
        # path-loss CSV on disk), silently refresh the Path-Loss viewer so
        # the operator can see before/after without an extra click.
        self._corr_page.correction_applied.connect(
...
<<<<<<< HEAD
    def _on_validation_error(self, msg):
        ...
        self._dash_page._console.setPlainText(msg)
...
<<<<<<< HEAD
    def _export_excel(self):
        ...
        dut = self._report.dut_serial or "report"
        path, _ = QFileDialog.getSaveFileName(
            self, "Export Excel", f"calibration_{dut}.xlsx", "Excel files (*.xlsx)"
        )
        if not path:
            return
        try:
            ...
            op = (self._user.full_name or self._user.matricule) if self._user else ""
            ExcelReport.generate(self._report, path, operator=op)
            QMessageBox.information(self, "Export complete", f"Excel saved to:\n{path}")
        except ImportError:
            QMessageBox.warning(self, "Missing dependency",
                              "openpyxl and pandas are required for Excel export.\n"
