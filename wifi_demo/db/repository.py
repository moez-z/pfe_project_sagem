...
# ── Password hashing ──────────────────────────────────────────────────────────
# Using SHA-256 + salt stored in the hash string.
# Format: "sha256$<salt_hex>$<hash_hex>"
# (No external dependency — bcrypt would be better in production)
...
class UserRepo:
    ...
    def create(db: DbSession, matricule: str, password: str,
        ...
        """Create a new user. Raises ValueError if matricule already exists."""
    ...
    def verify(db: DbSession, matricule: str, password: str) -> Optional[User]:
        """
        Returns the User if credentials are correct, None otherwise.
...
class SessionRepo:
    @staticmethod
    def save(db: DbSession, report, user_id: Optional[int] = None) -> int:
        """
        Save one calibration session + TX/RX child rows in one transaction.
        Uses the provided db session only.
        """
        try:
            ...
            db.commit()
            db.refresh(session)
            print(f"[db] saved calibration_session id={session.id}, user_id={session.user_id}")
            return session.id
    ...
    @staticmethod
    def save(db: DbSession, report, user_id: Optional[int] = None) -> int:
        from db.config import SessionLocal          # ← fresh session
        ...
        try:
            ...
            fresh_db.close()                        # ← always clean up
    ...
    def list_recent(db: DbSession, limit: int = 100,
        ...
        """Return recent sessions, newest first. Optionally filter by user."""
    ...
    def stats_summary(db: DbSession) -> dict:
        ...
        Returns a dict ready to display in metric cards.
        """
        ...
        # Pass rate per band — from tx_results
        band_stats = {}
    ...
    def recent_deltas(db: DbSession, limit: int = 30) -> list[dict]:
        """
        Returns the last N sessions with their avg delta — for a trend chart.
        """
        rows = (db.query(
...
class PostRepo:
    ...
    def ensure_defaults(db: DbSession):
        """Create posts 1-10 if they don't exist yet."""
