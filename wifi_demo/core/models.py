...
<<<<<<< HEAD
    ...
    # Each entry: (block_number, raw_header, reason)
    rejected_tx_blocks: list = field(default_factory=list)
    rejected_rx_blocks: list = field(default_factory=list)

    # --- Block headers explicitly marked "Skipped" by the test station ---
    # These are intentional (e.g. a band/antenna combo not tested this run)
    # and are informational only — they do NOT block calibration.
    # Each entry: (block_number, raw_header)
